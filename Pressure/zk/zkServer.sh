...
start)
    ...
    nohup $JAVA "-Xms512m" "-Xmx1g" "-Xmn256m" "-XX:PermSize=96m" "-XX:MaxPermSize=512m" "-XX:+UseConcMarkSweepGC" "-XX:+UseCMSCompactAtFullCollection" "-XX:+CMSClassUnloadingEnabled" "-XX:CMSMaxAbortablePrecleanTime=5000" "-XX:CMSInitiatingOccupancyFraction=80" "-XX:+HeapDumpOnOutOfMemoryError" "-XX:HeapDumpPath=${ZOO_LOG_DIR}/java.hprof" "-verbose:gc" "-Xloggc:${ZOO_LOG_DIR}/gc.log" "-XX:+PrintGCDetails" "-XX:+PrintGCDateStamps" "-Dzookeeper.log.dir=${ZOO_LOG_DIR}" "-Dzookeeper.root.logger=${ZOO_LOG4J_PROP}" \
